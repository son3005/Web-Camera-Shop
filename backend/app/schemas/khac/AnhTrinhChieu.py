...
class AnhTrinhChieuBase(BaseModel):
    ...
    hinh_anh_url: str = Field(..., max_length=500, description="URL hình ảnh của ảnh trình chiếu")
    public_id: str = Field(..., max_length=500, description="Public ID của hình ảnh trên dịch vụ lưu trữ")
    lien_ket: Optional[str] = Field(None, max_length=500, description="Liên kết khi người dùng nhấp vào ảnh trình chiếu")
    vi_tri: int = Field(..., description="Vị trí hiển thị của ảnh trình chiếu")
    trang_thai: bool = Field(..., description="Trạng thái hiển thị của ảnh trình chiếu")
...
class AnhTrinhChieuUpdate(BaseModel):
    ...
    trang_thai: Optional[bool] = Field(None, description="Trạng thái hiển thị của ảnh trình chiếu")
