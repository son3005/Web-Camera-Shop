...
from ..models.enums import TrangThaiAnhTrinhChieuEnum  # ⬅️ thêm import enum

class AnhTrinhChieuService:
    @staticmethod
    def get_anh_trinh_chieu_by_id(
        anh_trinh_chieu_id: int,
    ) -> Optional[AnhTrinhChieuResponsePrivate]:
    ...
    def get_all_anh_trinh_chieu_private() -> DanhSachAnhTrinhChieuResponsePrivate:
        # sắp xếp theo vị trí cho admin dễ nhìn
        anh_trinh_chieus = (
            AnhTrinhChieu.query.order_by(AnhTrinhChieu.vi_tri.asc()).all()
        )
        response = DanhSachAnhTrinhChieuResponsePrivate(
            anh_trinh_chieus=[
                AnhTrinhChieuResponsePrivate.from_orm(atc)
                for atc in anh_trinh_chieus
            ]
        )
        return response
    ...
    def get_all_anh_trinh_chieu_public() -> DanhSachAnhTrinhChieuResponsePublic:
        # ⬅️ CHỖ QUAN TRỌNG: chỉ lấy banner đang HIEU_LUC (enum), không xài True/False
        anh_trinh_chieus = (
            AnhTrinhChieu.query.filter_by(
                trang_thai=TrangThaiAnhTrinhChieuEnum.HIEU_LUC
            )
            .order_by(AnhTrinhChieu.vi_tri.asc())
            .all()
        )
        response = DanhSachAnhTrinhChieuResponsePublic(
            anh_trinh_chieus=[
                AnhTrinhChieuResponsePublic.from_orm(atc)
                for atc in anh_trinh_chieus
            ]
        )
        return response
    ...
    @staticmethod
    def update_anh_trinh_chieu(
        anh_trinh_chieu_id: int, data: AnhTrinhChieuUpdate
    ) -> Optional[AnhTrinhChieuResponsePrivate]:
        anh_trinh_chieu = AnhTrinhChieu.query.get(anh_trinh_chieu_id)
