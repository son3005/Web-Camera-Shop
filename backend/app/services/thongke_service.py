...
class ThongKeService:
    ...
    def thong_ke_doanh_thu_theo_thang_nam(nam=None, thang=None):
        ...
        - Doanh thu:   sum(chi_tiet_don_hang.don_gia_luc_mua * so_luong)
        - Giá vốn:     lấy GIÁ NHẬP TRUNG BÌNH cho từng biến thể
                       (từ bảng chi_tiet_phieu_nhap)
        - Lợi nhuận:   doanh thu - giá vốn

        Chỉ thống kê:
        - Đơn hàng có trạng thái ĐÃ GIAO
        - Thanh toán có trạng thái ĐÃ THANH TOÁN
        """
        try:
            # -------- Subquery: giá nhập trung bình theo từng biến thể --------
            sub_gia_nhap = (
                db.session.query(
                    ChiTietPhieuNhap.bien_the_san_pham_id.label("bt_id"),
                    func.avg(ChiTietPhieuNhap.gia_nhap_tung_vat).label("gia_nhap_tb"),
                )
                .group_by(ChiTietPhieuNhap.bien_the_san_pham_id)
                .subquery()
            )

            nam_col = extract("year", DonHang.ngay_tao)
            thang_col = extract("month", DonHang.ngay_tao)
            ...
            if thang is not None:
                query = (
                    db.session.query(
                        nam_col.label("nam"),
                        thang_col.label("thang"),
                        func.sum(
                            ChiTietDonHang.don_gia_luc_mua * ChiTietDonHang.so_luong
                        ).label("tong_doanh_thu"),
                        func.sum(
                            (
                                ChiTietDonHang.don_gia_luc_mua
                                - func.coalesce(sub_gia_nhap.c.gia_nhap_tb, 0)
                            )
                    ...
                    .outerjoin(
                        sub_gia_nhap,
                        sub_gia_nhap.c.bt_id == ChiTietDonHang.bien_the_san_pham_id,
                    )
                    .filter(
                ...
                if nam:
                    query = query.filter(nam_col == nam)

                query = query.group_by(nam_col, thang_col)
                rows = query.order_by(nam_col, thang_col).all()
                    ...
                    formatted_result.append(
                        {
                            "nam": int(row.nam),
                            "thang": int(row.thang),
                            "tong_doanh_thu": float(row.tong_doanh_thu or 0),
                            "loi_nhuan": float(row.loi_nhuan or 0),
                        }
                    )
                return formatted_result

            # ================== TRƯỜNG HỢP KHÔNG CÓ THÁNG ==================
            else:
                query = (
                    db.session.query(
                        nam_col.label("nam"),
                ...
                query = query.group_by(nam_col)
                rows = query.order_by(nam_col).all()
    ...
    def thong_ke_nguoi_dung_moi(nam=None, thang=None):
        ...
        try:
            ...
            if nam is None:
                ...
                ).filter(NguoiDung.ngay_tao.isnot(None)).order_by(
                    extract("year", NguoiDung.ngay_tao).desc()
                )
                ...
                for year in years:
                    ...
                    ).filter(extract("year", NguoiDung.ngay_tao) == year)

                    if thang:
                        count_query = count_query.filter(
                            extract("month", NguoiDung.ngay_tao) == thang
                        )
                ...
                query = db.session.query(
                    ...
                    func.count(NguoiDung.id).label("so_nguoi_dung_moi"),
                ).filter(extract("year", NguoiDung.ngay_tao) == nam)

                if thang:
                    query = query.filter(extract("month", NguoiDung.ngay_tao) == thang)
                    query = query.group_by(
                        extract("year", NguoiDung.ngay_tao),
                        extract("month", NguoiDung.ngay_tao),
                    )
                    result = query.all()
        ...
        try:
            ...
            if nam is None:
                ...
                for year in years:
                    ...
                    if thang:
                        count_query = count_query.filter(
                            extract("month", DonHang.ngay_tao) == thang
                        )
                ...
                query = db.session.query(
                    ...
                    func.count(DonHang.id).label("so_don_thanh_cong"),
                ).filter(DonHang.trang_thai == TrangThaiDonHangEnum.DA_GIAO)
    ...
    # ============================================================
    # 4. TỈ TRỌNG DOANH THU THEO THƯƠNG HIỆU
    # ============================================================
    @staticmethod
    def thong_ke_ti_trong_thuong_hieu(nam=None, thang=None):
        ...
        try:
            ...
            result = (
                ...
                .join(
                    BienTheSanPham,
                    ChiTietDonHang.bien_the_san_pham_id == BienTheSanPham.id,
                )
            ...
            tong_doanh_thu = sum(
                float(item.tong_doanh_thu) if item.tong_doanh_thu else 0
                for item in result
            )
                ...
                formatted_result.append(
                    {
                        ...
                        if item.tong_doanh_thu
                        else 0,
                        "ti_trong_phantram": (
                            float(item.tong_doanh_thu) / tong_doanh_thu * 100
                            if tong_doanh_thu > 0
                            else 0
                        ),
                    }
    ...
    def thong_ke_nguoi_dung_dang_nhap(nam=None, thang=None):
        ...
        try:
            ...
            if nam is None:
                years_query = db.session.query(
                    distinct(extract("year", NguoiDung.lan_cuoi_dang_nhap)).label(
                        "nam"
                    )
                ).filter(
                    NguoiDung.lan_cuoi_dang_nhap.isnot(None)
                ).order_by(
                    extract("year", NguoiDung.lan_cuoi_dang_nhap).desc()
                )
                ...
                for year in years:
                    ...
                    if thang:
                        count_query = count_query.filter(
                            extract("month", NguoiDung.lan_cuoi_dang_nhap) == thang
                        )
                ...
                query = db.session.query(
                    ...
                    func.count(NguoiDung.id).label("so_nguoi_dung_dang_nhap"),
                ).filter(NguoiDung.lan_cuoi_dang_nhap.isnot(None))

                if nam:
                    query = query.filter(
                        extract("year", NguoiDung.lan_cuoi_dang_nhap) == nam
                    )
                ...
                if thang:
                    ...
                    query = query.group_by(
                        extract("year", NguoiDung.lan_cuoi_dang_nhap),
                        extract("month", NguoiDung.lan_cuoi_dang_nhap),
                    )
                    result = query.all()
    ...
    def thong_ke_danh_gia(nam=None, thang=None):
        ...
        try:
            ...
            if nam is None:
                ...
                ).filter(DanhGia.ngay_tao.isnot(None)).order_by(
                    extract("year", DanhGia.ngay_tao).desc()
                )
                ...
                for year in years:
                    ...
                    query = db.session.query(
                        func.sum(
                            case((DanhGia.diem_danh_gia.in_([1, 2]), 1), else_=0)
                        ).label("danh_gia_tieu_cuc"),
                        func.sum(
                            case((DanhGia.diem_danh_gia.in_([3, 4]), 1), else_=0)
                        ).label("danh_gia_trung_binh"),
                        func.sum(
                            case((DanhGia.diem_danh_gia == 5, 1), else_=0)
                        ).label("danh_gia_tich_cuc"),
                        func.count(DanhGia.id).label("tong_so_danh_gia"),
                    ).filter(extract("year", DanhGia.ngay_tao) == year)

                    if thang:
                        query = query.filter(
                            extract("month", DanhGia.ngay_tao) == thang
                        )
                        ...
                        formatted_result.append(
                            {
                                ...
                                "danh_gia_trung_binh": result_item.danh_gia_trung_binh
                                or 0,
                ...
                if thang:
                    ...
                    query = query.group_by(
                        extract("year", DanhGia.ngay_tao),
                        extract("month", DanhGia.ngay_tao),
                    )
                    result = query.all()
    ...
    def tong_hop_thong_ke():
        ...
        QUAN TRỌNG:
        - Doanh thu tháng này: dùng CHUNG công thức với biểu đồ doanh thu
          + Chỉ đơn hàng ĐÃ GIAO (DA_GIAO)
          + Thanh toán ĐÃ THANH TOÁN (DA_THANH_TOAN)
          + Tính theo chi_tiet_don_hang: don_gia_luc_mua * so_luong
        """
        try:
            thang_nay = datetime.now().month
            nam_nay = datetime.now().year

            # --- Doanh thu tháng này: cùng logic với biểu đồ ---
            doanh_thu_thang_nay = (
                db.session.query(
                    func.coalesce(
                        func.sum(
                ...
                .join(ThanhToan, ThanhToan.don_hang_id == DonHang.id)
                .filter(
                    DonHang.trang_thai == TrangThaiDonHangEnum.DA_GIAO,
            ...
            # --- Đơn hàng tháng này: cũng chỉ tính đơn đã giao & đã thanh toán ---
            don_hang_thang_nay = (
                db.session.query(func.count(func.distinct(DonHang.id)))
                .join(ThanhToan, ThanhToan.don_hang_id == DonHang.id)
                .filter(
                    DonHang.trang_thai == TrangThaiDonHangEnum.DA_GIAO,
                    ThanhToan.trang_thai == TrangThaiThanhToanEnum.DA_THANH_TOAN,
            ...
            # --- Người dùng mới tháng này (giữ nguyên logic cũ) ---
            nguoi_dung_moi = (
                db.session.query(func.count(NguoiDung.id))
                .filter(
            ...
            return {
                "doanh_thu_thang_nay": float(doanh_thu_thang_nay or 0),
