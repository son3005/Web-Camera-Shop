...
export default function CartPage() {
  ...
  const handleCheckout = () => {
    ...
    const checkoutData = selectedItems.map((it) => {
      // ✅ cố gắng lấy tồn kho từ nhiều field khác nhau
      let ton_kho = null;

      if (typeof it.ton_kho === "number") {
        ton_kho = it.ton_kho;
      ...
      return {
        ...
        ton_kho, // 🔥 thêm tồn kho để CheckoutPage dùng
      };
    });

    // ⭐ TRUYỀN THÊM gio_hang_id SANG CHECKOUT
    navigate("/checkout", {
      state: {
        items: checkoutData,
        gio_hang_id: cart?.id || null, // ==> để backend biết đây là mua từ giỏ
      },
    });
  };
