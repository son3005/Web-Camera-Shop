// src/components/common/PhieuNhap/PhieuNhapDetailModal.jsx
// ======================================================
// Modal xem chi tiết 1 phiếu nhập
// ======================================================

import { useQuery } from "@tanstack/react-query";
import { getPhieuNhap } from "../../../api/phieuNhapApi";

export default function PhieuNhapDetailModal({ phieuNhapId, onClose }) {
  const {
    data: pt,
    isLoading,
    isError,
  } = useQuery({
    queryKey: ["phieu-nhap-detail", phieuNhapId],
    queryFn: () => getPhieuNhap(phieuNhapId),
  });

  const ngayNhap = pt?.ngay_nhap || pt?.ngay_tao || pt?.ngay_cap_nhat || null;
  const chiTiet =
    pt?.chi_tiet ||
    pt?.cac_chi_tiet_phieu_nhap ||
    pt?.phieu_nhap_chi_tiets ||
    pt?.chi_tiet_phieu_nhap ||
    [];

  const tongTien =
    pt?.tong_gia_tri ??
    chiTiet.reduce((sum, item) => {
      const sl = Number(item.so_luong || 0);
      const gia = Number(item.gia_nhap_tung_vat || 0);
      return sum + sl * gia;
    }, 0);

  const formatVnd = (n) =>
    Number(n || 0).toLocaleString("vi-VN", {
      maximumFractionDigits: 0,
    }) + "₫";

  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl max-h-[90vh] rounded-2xl border border-emerald-50 bg-white/96 text-slate-900 shadow-2xl flex flex-col overflow-hidden">
        {/* header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white/90">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">
              Chi tiết phiếu nhập{" "}
              {pt?.ma_phieu_nhap ? `– ${pt.ma_phieu_nhap}` : ""}
            </h2>
            {ngayNhap && (
              <p className="text-xs text-slate-500">
                Ngày nhập: {new Date(ngayNhap).toLocaleString("vi-VN")}
              </p>
            )}
          </div>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-700 text-xl cursor-pointer"
          >
            ✕
          </button>
        </div>

        {/* body */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {isLoading && (
            <div className="text-sm text-slate-600">Đang tải chi tiết...</div>
          )}
          {isError && (
            <div className="text-sm text-red-500">
              Không tải được chi tiết phiếu nhập
            </div>
          )}

          {!isLoading && !isError && pt && (
            <>
              {/* info chung */}
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="space-y-1">
                  <p className="text-slate-500">Mã phiếu</p>
                  <p className="font-medium">{pt.ma_phieu_nhap || "-"}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-slate-500">Nhà cung cấp</p>
                  <p className="font-medium">
                    {pt.ten_nha_cung_cap || pt.ten || "-"}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-slate-500">Ngày nhập</p>
                  <p className="font-medium">
                    {ngayNhap
                      ? new Date(ngayNhap).toLocaleString("vi-VN")
                      : "-"}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-slate-500">Tổng giá trị</p>
                  <p className="font-semibold text-emerald-600">
                    {formatVnd(tongTien)}
                  </p>
                </div>
              </div>

              {/* bảng chi tiết */}
              <div className="rounded-xl overflow-hidden border border-slate-200 bg-white/90">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 text-slate-700">
                    <tr>
                      <th className="p-2 text-left font-semibold">#</th>
                      <th className="p-2 text-left font-semibold">Biến thể</th>
                      <th className="p-2 text-right font-semibold">SL</th>
                      <th className="p-2 text-right font-semibold">Giá nhập</th>
                      <th className="p-2 text-right font-semibold">
                        Thành tiền
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {chiTiet.map((ct, idx) => {
                      const sl = Number(ct.so_luong || 0);
                      const gia = Number(ct.gia_nhap_tung_vat || 0);
                      const thanhTien = sl * gia;

                      const tenBienThe =
                        ct.ten_bien_the ||
                        (ct.ten_san_pham
                          ? `${ct.ten_san_pham}${
                              ct.ten_bien_the ? ` – ${ct.ten_bien_the}` : ""
                            }`
                          : null) ||
                        `Biến thể #${ct.bien_the_san_pham_id}`;

                      return (
                        <tr
                          key={ct.id || idx}
                          className="border-t border-slate-100 hover:bg-slate-50/80 transition-colors"
                        >
                          <td className="p-2">{idx + 1}</td>
                          <td className="p-2">{tenBienThe}</td>
                          <td className="p-2 text-right">{sl}</td>
                          <td className="p-2 text-right">{formatVnd(gia)}</td>
                          <td className="p-2 text-right">
                            {formatVnd(thanhTien)}
                          </td>
                        </tr>
                      );
                    })}

                    {chiTiet.length === 0 && (
                      <tr>
                        <td
                          colSpan={5}
                          className="p-4 text-center text-slate-500"
                        >
                          Phiếu chưa có dòng chi tiết
                        </td>
                      </tr>
                    )}
                  </tbody>

                  {chiTiet.length > 0 && (
                    <tfoot className="bg-slate-50">
                      <tr>
                        <td
                          colSpan={4}
                          className="p-2 text-right font-semibold"
                        >
                          Tổng
                        </td>
                        <td className="p-2 text-right font-bold text-emerald-600">
                          {formatVnd(tongTien)}
                        </td>
                      </tr>
                    </tfoot>
                  )}
                </table>
              </div>
            </>
          )}
        </div>

        {/* footer */}
        <div className="px-6 py-3 border-t border-slate-200 bg-white/80 text-right">
          <button
            onClick={onClose}
            className="inline-flex items-center justify-center px-4 py-2 rounded-lg border border-slate-200 bg-white/90 text-slate-700 hover:bg-slate-100 cursor-pointer transition"
          >
            Đóng
          </button>
        </div>
      </div>
    </div>
  );
}
