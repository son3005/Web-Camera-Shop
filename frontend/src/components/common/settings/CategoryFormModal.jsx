...
const CategoryFormModal = ({ open, onClose, onSubmit, defaultValues }) => {
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white text-slate-900 rounded-3xl p-5 shadow-2xl border border-slate-200">
        ...
        <form onSubmit={handleSubmit((v) => onSubmit(v))} className="space-y-4">
          <div>
            ...
            <input
              className="w-full mt-1 px-3 py-2 rounded-lg bg-white border border-slate-300 text-slate-900 placeholder:text-slate-400 shadow-inner
          ...
          <div>
            <label className="text-sm text-slate-700">Tên danh mục *</label>
            <input
              className="w-full mt-1 px-3 py-2 rounded-lg bg-white border border-slate-300 text-slate-900 placeholder:text-slate-400 shadow-inner
                         focus:outline-none focus:ring-2 focus:ring-emerald-500/70 focus:border-emerald-500"
          ...
          <div className="pt-2 flex justify-end gap-2">
            ...
            <button
              type="submit"
              className="px-4 py-2 rounded-lg text-white bg-gradient-to-r from-emerald-500 to-slate-600 hover:from-emerald-400 hover:to-slate-500 shadow-md hover:shadow-lg cursor-pointer transition"
            >
