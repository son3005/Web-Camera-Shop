    ...
    <div className="space-y-4">
      ...
      <div className="p-5 rounded-2xl shadow-lg bg-white/20 dark:bg-slate-700/30 backdrop-blur-sm border border-white/20 dark:border-slate-600/50 max-h-[550px] overflow-y-auto space-y-6 scrollbar-thin scrollbar-thumb-gray-400/50 dark:scrollbar-thumb-gray-500/50">
        {Object.entries(propertyGroups).map(([groupName, props]) => {
          const groupKey = groupName.toLowerCase(); // Ví dụ: "Ánh sáng" -> "ánh sáng"
          return (
            <div key={groupName} className="space-y-4">
              ...
              <div className="space-y-3">
                {props.map((prop) => {
                  if (readOnly) {
                    // Chỉnh sửa: Sử dụng thong_so_ky_thuat thay vì properties
                    const value = properties?.thong_so_ky_thuat?.[groupKey]?.[prop.key] || 'N/A'; // Lưu ý: properties ở đây là props từ Controller, cần rename nếu cần
                    return (
                  ...
                  return (
                    <PropertyInput
                      key={prop.key}
                      control={control}
                      // Chỉnh sửa: Thay properties bằng thong_so_ky_thuat để khớp schema Backend
                      name={`thong_so_ky_thuat.${groupKey}.${prop.key}`}
                      label={prop.label}
                    />
                  );
                })}
