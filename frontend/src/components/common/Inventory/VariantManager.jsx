                    ...
                    {(defaultVariants || []).map((variant, index) => (
                        <div key={variant.id || index} className="p-4 rounded-2xl bg-white/20 dark:bg-slate-700/30 backdrop-blur-sm border border-white/20 dark:border-slate-600/50 space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                ...
                                <FormInput label="Tồn kho" type="number" readOnly={true} defaultValue={variant.so_luong_ton} />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-800 dark:text-slate-200 mb-2">Hình ảnh</label>
                                <VariantImageUpload images={variant.hinh_anhs || []} readOnly={true} />
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="space-y-6 max-h-[400px] overflow-y-auto pr-4 -mr-4 scrollbar-thin">
                {fields.map((field, index) => (
                    <div key={field.id} className="p-4 rounded-2xl bg-white/20 dark:bg-slate-700/30 backdrop-blur-sm border border-white/20 dark:border-slate-600/50 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <FormInput label="Tên biến thể" name={`bien_the_san_phams.${index}.ten_bien_the`} register={register} errors={errors?.bien_the_san_phams?.[index]?.ten_bien_the} />
                        </div>
                        <div className="grid grid-cols-3 gap-4">
                            <FormInput label="Giá bán" type="number" name={`bien_the_san_phams.${index}.gia_ban`} register={register} errors={errors?.bien_the_san_phams?.[index]?.gia_ban} />
                            <FormInput label="Giá khuyến mãi" type="number" name={`bien_the_san_phams.${index}.gia_khuyen_mai`} register={register} errors={errors?.bien_the_san_phams?.[index]?.gia_khuyen_mai} />
                            <FormInput label="Tồn kho" type="number" name={`bien_the_san_phams.${index}.so_luong_ton`} register={register} errors={errors?.bien_the_san_phams?.[index]?.so_luong_ton} />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-800 dark:text-slate-200 mb-2">Hình ảnh</label>
                            <Controller 
                                ...
                                render={({ field: { onChange, value } }) => (
                                    <VariantImageUpload images={value} onChange={onChange} />
                                )} 
                            />
                        </div>
                    </div>
