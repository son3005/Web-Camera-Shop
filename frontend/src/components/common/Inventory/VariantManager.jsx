...
const DateInput = ({ label, name, register, errors, readOnly, defaultValue }) => (
    <div className="flex-1 min-w-[120px]">
        <label className="block text-sm font-medium text-slate-800 dark:text-slate-200 mb-1">{label}</label>
        <input
            type="datetime-local"
            ...
            defaultValue={readOnly ? defaultValue : undefined}
            {...(register ? register(name) : {})}
            className={`w-full rounded-lg px-3 py-2 text-sm transition-all bg-white/50 dark:bg-slate-700/50 border ${errors ? 'border-red-500 focus:ring-red-500' : 'border-black/10 dark:border-white/10 focus:ring-cyan-500'} focus:outline-none focus:ring-2 ${readOnly ? 'cursor-not-allowed bg-slate-200 dark:bg-slate-700/30' : ''}`}
        />
        {errors && <p className="text-red-500 text-xs mt-1 h-4">{errors.message}</p>}
    </div>
);

const VariantManager = ({ control, register, errors, defaultVariants, readOnly = false, uploadProgress, placeholderImage }) => {
    if (readOnly) {
        return (
                    ...
                    {(defaultVariants || []).map((variant, index) => (
                        <div key={variant.id || index} className="p-4 rounded-2xl bg-white/20 dark:bg-slate-700/30 backdrop-blur-sm border border-white/20 dark:border-slate-600/50 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <FormInput label="Tên biến thể" readOnly={true} defaultValue={variant.ten_bien_the} />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <FormInput label="Giá bán" type="number" readOnly={true} defaultValue={variant.gia_ban} />
                                <FormInput label="Giá khuyến mãi" type="number" readOnly={true} defaultValue={variant.gia_khuyen_mai} />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <DateInput label="Ngày bắt đầu KM" readOnly={true} defaultValue={variant.ngay_bat_dau_khuyen_mai} />
                                <DateInput label="Ngày kết thúc KM" readOnly={true} defaultValue={variant.ngay_ket_thuc_khuyen_mai} />
                            </div>
                            <div className="grid grid-cols-1 gap-4">
                                <FormInput label="Số lượng tồn" type="number" readOnly={true} defaultValue={variant.so_luong_ton} />
                            </div>
                            <div>
                    ...
                    <div key={field.id} className="p-4 rounded-2xl bg-white/20 dark:bg-slate-700/30 backdrop-blur-sm border border-white/20 dark:border-slate-600/50 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <DateInput 
                                ...
                                register={register} 
                                errors={errors?.bien_the_san_phams?.[index]?.ngay_bat_dau_khuyen_mai} 
                            />
                            <DateInput 
                                label="Ngày kết thúc KM" 
                                name={`bien_the_san_phams.${index}.ngay_ket_thuc_khuyen_mai`} 
                                register={register} 
                                errors={errors?.bien_the_san_phams?.[index]?.ngay_ket_thuc_khuyen_mai} 
                            />
                        </div>
                        <div className="grid grid-cols-1 gap-4">
                            <FormInput 
                        ...
                        <div>
                            ...
                            <Controller 
                                ...
                                render={({ field: { value, onChange } }) => {
                                    console.log(`VariantManager - Controller for variant ${index}:`, value);
                                    return (
                                        <VariantImageUpload 
                                            images={value || []} 
                                            onChange={(newImages) => {
                                                console.log(`VariantManager - Images changed for variant ${index}:`, newImages);
                                                onChange(newImages);
                                            }} 
